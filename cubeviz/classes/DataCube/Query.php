...
{    
    ...
    public function getComponents($dsdUri, $dsUri, $componentType) {
        ...
        foreach ($tmp as $dimension) {
            $dimension ["__properties"] = $dimension["http://purl.org/linked-data/cube#dimension"];

            $dimension ['__cv_elements'] = $this->getComponentElements(
                $dsUri, $dimension[$componentType]
            );
           if ($dimension ["__properties"]!=null){
			$dimension ['hierarchy'] = $this->getHierarchy(
                $dsUri, $dimension[$componentType], $dimension ['__cv_elements']
            	);
           }
     ...
     * @return array
     */
	public function getHierarchy($dataSetUri, $componentProperty, $dimensions) 
	{
		$procurar = array();
		$achou = array();
		foreach($dimensions as $dimension){
			array_push($procurar, $dimension['__cv_uri']);
		}
		...
		$posicao = 0;
		while(sizeof($procurar)>$posicao){
			$elementoUri = $procurar[$posicao];
			$posicao = $posicao+1;
			$elemento = $mapa[$elementoUri];
			$elemento["__has_upper"]=false;
			if (!in_array($elemento["http://purl.obolibrary.org/obo#part_of"], $procurar)) { 
				if ($elemento["http://purl.obolibrary.org/obo#part_of"]!=""){
					$result2 = $this->_model->sparqlQuery('SELECT ?componentUri ?p ?o WHERE {
						<'.$elemento["http://purl.obolibrary.org/obo#part_of"].'> ?p ?o .
						OPTIONAL {
							?componentUri ?p ?o.
						}
					}');
					$result2 = $this->generateAssocSPOArrayFromSparqlResult($result2, 'componentUri', 'p', 'o');
					$result2 = $this->enrichResult($result2);
					foreach($result2 as $r2){
						if ($r2["__cv_uri"]==$elemento["http://purl.obolibrary.org/obo#part_of"]){
							$mapa[$r2["__cv_uri"]] = $r2;
							array_push($procurar,$r2["__cv_uri"]);
						}
					}
				}
			}
			if ($elemento["http://purl.obolibrary.org/obo#part_of"]!=""){
				$elemento["__has_upper"]=true;
			}
			array_push($array, $elemento);
		}
		$res = array();
		foreach($array as $i => $d){
			$has_upper = $array[$i]["__has_upper"]; 
			if ($has_upper==false){
				$array[$i]["__elements"] = $this->geraArvore($array[$i],$array); 
    ...
    public function geraArvore($elemento, $array){
	$resposta = array();
	foreach($array as $i => $d){
		if ($array[$i]["http://purl.obolibrary.org/obo#part_of"]==$elemento["__cv_uri"]){
			$array[$i]["__elements"] = $this->geraArvore($array[$i],$array);
			array_push($resposta,$array[$i]);
		}
	}
	return $resposta;
    }
